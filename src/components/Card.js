export class Card {
  ...
  _getTemplate() {
    //выбор темплейта
    const cardElement = (this._cardUserId === this._userId.getUserInfo().userId) ? document.querySelector(`${this._cardSelector}_delete_present`).content.cloneNode(true) : document.querySelector(`${this._cardSelector}_delete_empty`).content.cloneNode(true)
  ...
  _setEventListeners() {
    ...
    if(this._cardUserId === this._userId.getUserInfo().userId) {
      this._element.querySelector('.element__delete').addEventListener('click', () => this._handleDeleteClick(this._cardId, this._cardElementSelector));}
